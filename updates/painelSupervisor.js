...
function createPainel (setor, colaboradores, condiction) {
    ...
    const qtdDias = 8
    const qtdViews = 3
    const { datas, dias } = getArrayDate(qtdDias,qtdViews)
    
    const tablesArray = generateTable(colaboradores, datas, dias)

    //const htmlTables = tablesArray.join("")
    ...
    li.addEventListener('click', event => {
        ...
        const isOpened = li.classList.contains("open")
...
function hiddeBackground(background) {
    background.remove()
}

function showBackground(background, li) {
    background.addEventListener('click', () => {
        hiddePanel(li)
        hiddeBackground(background)
    })
}
...
function hiddePanel(li) {
    li.classList.add("hidde-xs")
    li.classList.remove("open")
}

function showPanel(li) {
    li.classList.remove("hidde-xs")
    li.classList.add("open")

    const background = createBackground()

    showBackground(background, li)
}